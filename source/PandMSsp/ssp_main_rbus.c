...
void *dlhandle = NULL;
ULONG g_currentBsUpdate = 3;
static int load_plugin_users();
static int unload_plugin_users();
static int ssp_main_load_plugin();
static int load_plugin_deviceinfo();
static int unload_plugin_deviceinfo();
...
int  cmd_dispatch(int  command)
{
    ...
    switch ( command )
    {
		...
		fprintf(stderr, "I am inside cmd dispatcj");
		...
		/* ?? dpn't find easy way to ignore this,have to check later*/
		...
		bus_info->mallocfunc = malloc;
		bus_info->freefunc = free;
		strncpy(bus_info->component_id, CName , sizeof(bus_info->component_id) -1);
                bus_info->component_id[sizeof(bus_info->component_id)-1] = '\0';
		CcspTraceInfo(("My component id:%s\n",bus_info->component_id));
		/* Have to Invoke this, to staisfy few folder get from psm.*/
		CCSP_Msg_IsRbus_enabled();
		rbusHandle_t rbusHandle = NULL;
		rc = rbus_open(&rbusHandle, CName);
                if(rc != RBUS_ERROR_SUCCESS)
    		{
                    return ANSC_STATUS_FAILURE;
                }
		bus_info->rbus_handle = rbusHandle;
...
void sig_handler(int sig)
{
    ...
    if ( sig == SIGINT ) {
        ...
        /* collect value */
        char buf[32] = {0};
        if( syscfg_get( NULL, "pam_so", buf, sizeof(buf)) == 0)
        {
            if (strcmp(buf, "plugin_load") == 0)
            {
                ssp_main_load_plugin();
            }
            else if (strcmp(buf, "users_load") == 0)
            {
                load_plugin_users();
            }
            else if (strcmp(buf, "users_unload") == 0)
            {
                unload_plugin_users();
            }
            else if (strcmp(buf, "deviceinfo_load") == 0)
            {
                load_plugin_deviceinfo();
            }
            else if (strcmp(buf, "deviceinfo_unload") == 0)
            {
                unload_plugin_deviceinfo();
            }
        }
    }
    else if ( sig == SIGUSR2 ) {
...
*/
    strcpy_s(gpPnmStartCfg->ComponentId, sizeof(gpPnmStartCfg->ComponentId),"com.cisco.spvtg.ccsp.pam");
    strcpy_s(gpPnmStartCfg->ComponentName, sizeof(gpPnmStartCfg->ComponentName),"com.cisco.spvtg.ccsp.pam");
    gpPnmStartCfg->Version = 1;
    strcpy_s(gpPnmStartCfg->DbusPath, sizeof(gpPnmStartCfg->DbusPath),"/com/cisco/spvtg/ccsp/pam");
    strcpy_s(gpPnmStartCfg->DmXmlCfgFileName, sizeof(gpPnmStartCfg->DmXmlCfgFileName),"CcspDmLib.cfg");

    fprintf(stderr,"I am inside new main");
...
static int ssp_main_load_plugin()
{
                int (*ptr_plugin_init)(CCSP_MESSAGE_BUS_INFO *);

                dlhandle = dlopen("/usr/lib/libCcspPandM_plugin_rbus.so", RTLD_GLOBAL|RTLD_NOW);
                //dlhandle = dlopen("/var/tmp/libCcspPandM_plugin_rbus.so", RTLD_GLOBAL|RTLD_NOW);

                if (!dlhandle) {
                        CcspTraceError(("Error loading library: %s\n", dlerror()));
                        return ANSC_STATUS_FAILURE;
                }
                CcspTraceWarning(("After dlopen /usr/lib/libCcspPandM_rbus_wrapper_users.so\n"));

                ptr_plugin_init = dlsym(dlhandle, "plugin_init_rbus");

                if (!ptr_plugin_init) {
                   CcspTraceError(("Error getting function pointer: %s\n", dlerror()));
                   dlclose(dlhandle);
                   return ANSC_STATUS_FAILURE;
                }

                ptr_plugin_init(bus_handle);
...
static int load_plugin_users()
{
                CCSP_MESSAGE_BUS_INFO *bus_info        = bus_handle;
                int (*ptr_plugin_rbus_handle_event)(rbusHandle_t);
                ptr_plugin_rbus_handle_event = dlsym(dlhandle, "plugin_rbus_handle_event");
                if (!ptr_plugin_rbus_handle_event) {
                   CcspTraceError(("Error getting function pointer: %s\n", dlerror()));
                   dlclose(dlhandle);
                   return ANSC_STATUS_FAILURE;
                }
                ptr_plugin_rbus_handle_event(bus_info->rbus_handle);
        return 0;
}
...
{
                CCSP_MESSAGE_BUS_INFO *bus_info        = bus_handle;
                int (*ptr_plugin_rbus_handle_event_unload)(rbusHandle_t);
                ptr_plugin_rbus_handle_event_unload = dlsym(dlhandle, "plugin_rbus_handle_event_unload");
                if (!ptr_plugin_rbus_handle_event_unload) {
                   CcspTraceError(("Error getting function pointer: %s\n", dlerror()));
                   dlclose(dlhandle);
                   return ANSC_STATUS_FAILURE;
                }
                ptr_plugin_rbus_handle_event_unload(bus_info->rbus_handle);
        return 0;
}
...
{
                CCSP_MESSAGE_BUS_INFO *bus_info        = bus_handle;
                int (*ptr_plugin_deviceinfo_handle_event)(rbusHandle_t);
                ptr_plugin_deviceinfo_handle_event = dlsym(dlhandle, "plugin_deviceinfo_handle_event");
                if (!ptr_plugin_deviceinfo_handle_event) {
                   CcspTraceError(("Error getting function pointer: %s\n", dlerror()));
                   dlclose(dlhandle);
                   return ANSC_STATUS_FAILURE;
                }
                ptr_plugin_deviceinfo_handle_event(bus_info->rbus_handle);
        return 0;
}
...
{
                CCSP_MESSAGE_BUS_INFO *bus_info        = bus_handle;
                int (*ptr_plugin_deviceinfo_handle_event_unload)(rbusHandle_t);
                ptr_plugin_deviceinfo_handle_event_unload = dlsym(dlhandle, "plugin_deviceinfo_handle_event_unload");
                if (!ptr_plugin_deviceinfo_handle_event_unload) {
                   CcspTraceError(("Error getting function pointer: %s\n", dlerror()));
                   dlclose(dlhandle);
                   return ANSC_STATUS_FAILURE;
                }
                ptr_plugin_deviceinfo_handle_event_unload(bus_info->rbus_handle);
        return 0;
}
